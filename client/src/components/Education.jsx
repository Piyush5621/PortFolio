...
const Education = () => {
  ...
  return (
    <section id="education" className="py-24 bg-[var(--bg-cosmic)] relative overflow-hidden border-t border-white/5">
      ...
      {/* Background Ambience */}
      <div className="absolute inset-0 pointer-events-none opacity-[0.02]" style={{ backgroundImage: 'linear-gradient(rgba(230,167,0,0.5) 1px, transparent 1px), linear-gradient(90deg, rgba(230,167,0,0.5) 1px, transparent 1px)', backgroundSize: '60px 60px' }} />
      <div className="absolute bottom-[0%] right-[10%] w-[600px] h-[600px] bg-[rgba(230,167,0,0.02)] rounded-full blur-[160px] pointer-events-none" />
        ...
        <div className="mb-20 grid lg:grid-cols-2 gap-12 items-end">
            <div>
                <div className="inline-flex items-center gap-3 px-4 py-2 bg-[rgba(10,10,14,0.6)] backdrop-blur border border-white/10 rounded-full mb-6 relative cursor-default">
                    ...
                    <span className="text-[10px] font-mono text-slate-300 tracking-[0.2em] uppercase">Chronological Log</span>
                </div>
                <h2 className="text-5xl md:text-7xl font-black text-white uppercase tracking-tighter leading-[0.9]">
                    Academic <br/>
                    <span className="text-gold-gradient bg-clip-text text-transparent pb-2 block">Timeline.</span>
                </h2>
            </div>
            <div className="pb-4">
                <p className="text-sm font-medium text-slate-400 leading-relaxed border-l-2 border-[#E6A700]/50 pl-5">
                    A formalized record of my engineering education, degrees, and verified global certifications.
                </p>
            </div>
        </div>

        {/* ── Vertical Timeline Registry ── */}
        <div className="relative mb-32 max-w-4xl mx-auto lg:mx-0">
          
          {/* The Neural Spine (Vertical Line) */}
          <div className="absolute left-[20px] md:left-[24px] top-4 bottom-4 w-px bg-gradient-to-b from-[#E6A700]/50 via-[#E6A700]/20 to-transparent" />
          ...
          <div className="grid md:grid-cols-2 gap-6 relative z-10 w-full">
            {certifications.map((cert, idx) => (
              ...
              >
                ...
                <div className="relative z-10 flex shrink-0 border border-white/10 hover:border-[#E6A700] bg-white/[0.02] hover:bg-[#E6A700] text-slate-400 hover:text-black transition-all p-3 rounded-full items-center justify-center lg:self-center w-max">
